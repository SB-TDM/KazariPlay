...
(function () {
    ...
    window.updateSubtitlePos = function (x, y) {
        ...
        if (vx)
            vx.value = String(Math.round(subStyle.pos_x * 100));
        if (vy)
            vy.value = String(Math.round(subStyle.pos_y * 100));
        const lx = $('setSubPosXV'), ly = $('setSubPosYV');
        if (lx)
            lx.textContent = vx ? vx.value + '%' : '';
        if (ly)
            ly.textContent = vy ? vy.value + '%' : '';
        subToast('字幕位置已更新');
    };
    function subApplyStyle(s) {
        ...
        $('setSubBgAlpha').value = String(Math.round((subStyle.bg_a ?? 0.72) * 100));
        $('setSubCorner').value = String(subStyle.corner ?? 10);
        $('setSubPadding').value = String(subStyle.padding ?? 14);
        $('setSubBorderW').value = String(subStyle.border_w ?? 1.5);
        $('setSubFontSize').value = String(subStyle.font_size ?? 22);
        $('setSubFontWeight').value = String(subStyle.font_weight ?? 700);
        $('setSubOutlineW').value = String(subStyle.outline_w ?? 1.5);
        $('setSubShadowOff').value = String(subStyle.shadow_off ?? 2);
        $('setSubLineGap').value = String(subStyle.line_gap ?? 4);
        $('setSubMaxWidth').value = String(Math.round((subStyle.max_width ?? 0.9) * 100));
        $('setSubPosX').value = String(Math.round((subStyle.pos_x ?? 0.5) * 100));
        $('setSubPosY').value = String(Math.round((subStyle.pos_y ?? 0.82) * 100));
        $('setSubAvoidPx').value = String(subStyle.avoid_bottom_px ?? 60);
        if (subStyle.font) {
            const sel = $('setSubFontSel');
            const opt = Array.from(sel.options).find((o) => o.value === subStyle.font);
            if (opt)
                sel.value = subStyle.font;
        }
    ...
    function subBindControls() {
        ...
        ($('setSubSavePreset')).addEventListener('click', () => {
            ...
            bridge.saveSubtitlePreset(name, JSON.stringify(subStyle), function (r) {
                let res = {};
                try {
                    res = JSON.parse(String(r || '{}'));
                }
                catch (e) { }
                if (res && res.ok) {
                    bridge.getSubtitleStylePresets(function (p) {
                        try {
                            subPresets = JSON.parse(String(p || '{}'));
                        }
                        catch (e) {
                            subPresets = {};
                        }
        ...
        ($('setSubLoad')).addEventListener('click', () => {
            bridge.getSubtitleStyle(function (s) {
                try {
                    subApplyStyle(JSON.parse(String(s || '{}')));
                }
                catch (e) { }
                subToast('已加载已保存配置');
            });
    ...
    function subLoadConfig() {
        if (!bridge)
            return;
        Promise.all([
            new Promise((res) => bridge.getSubtitleStyle((s) => res(String(s || '{}')))),
            new Promise((res) => bridge.getSubtitleStylePresets((s) => res(String(s || '{}')))),
        ]).then(([s, p]) => {
            try {
                subApplyStyle(JSON.parse(s));
            }
            catch (e) { }
            try {
                subPresets = JSON.parse(p);
            }
            catch (e) {
                subPresets = {};
            }
