...
function renderCollectionGroup(group) {
    ...
    hdr.onkeydown = (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            selectCollection(group.id, group);
        }
    };
    ...
    if (tgl)
        tgl.addEventListener('click', (e) => { e.stopPropagation(); toggleExpand(group); });
    return el;
}
...
function bindCollectionItems() {
    document.querySelectorAll('#collectionTree .collection-item').forEach(it => {
        ...
        it.onkeydown = (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                selectCollection(cid, findCollectionNode(cid));
            }
        };
        it.oncontextmenu = (e) => {
            e.preventDefault();
            ...
            if (node)
                showCollectionCtx(e.clientX, e.clientY, node);
        };
    });
}
...
function clearCollectionFilter() {
    ...
    App.ui.state.collectionGroupId = null;
    App.ui.state.openGroupId = null;
    App.ui.state.nav = '全部作品';
    App.ui.state.kw = '';
    renderCollectionTree();
...
function newCollection(parentId) {
    showInputDialog({
        ...
        cb: function (name) {
            if (name && name.trim()) {
                bridge.createCollection(name.trim(), parentId || 0, '', '');
                toast('已创建');
            }
        }
    });
    ...
    showInputDialog({
        ...
        cb: function (name) {
            if (name && name.trim() && name.trim() !== node.name) {
                bridge.updateCollection(node.id, JSON.stringify({ name: name.trim() }));
                toast('已重命名');
            }
        }
    });
...
function openCollectionManager() {
    if (!App.data.currentGame)
        return;
    // 异步拉取最新数据（右键进入时 currentGame 可能是快照），再渲染
    bridge.getGame(String(App.data.currentGame.id), function (s) {
        try {
...
function renderGameCollections() {
    ...
    all.forEach(col => {
        ...
        c.onclick = () => {
            ...
            if (had) {
                g.collections = g.collections.filter(x => x.id !== col.id);
            }
            else {
                g.collections = g.collections.concat([{ id: col.id, name: col.label, color: col.color || '', icon: col.icon || '' }]);
            }
...
document.getElementById('btnNewCollection').onclick = () => {
    showInputDialog({
        title: '新建收藏夹',
        label: '名称',
        cb: function (name) { if (name && name.trim())
            bridge.createCollection(name.trim(), 0, '', ''); }
    });
};
