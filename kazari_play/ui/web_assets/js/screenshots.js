...
function renderScreenshots() {
    ...
    if (!grid || !App.data.currentGame)
        return;
    grid.innerHTML = '';
    bridge.getScreenshots(String(App.data.currentGame.id), function (s) {
        let shots = [];
        try {
            shots = JSON.parse(String(s || '[]'));
        }
        catch (e) { }
        if (!shots.length) {
...
function refreshScreenshots(gameId) {
    if (!App.data.currentGame || App.data.currentGame.id !== gameId)
        return;
