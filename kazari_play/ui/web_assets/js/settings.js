...
(function () {
  ...
  function loadConfig() {
    ...
    bridge.getConfig(function (s) {
      ...
      if (typeof subEnabled === 'boolean') $('setSubtitleEnabled').checked = subEnabled;
      const cln = cfg.clean || {};
  ...
  function takenHotkeys(exceptId) {
    ...
    const taken = new Set();
    ids.forEach((id) => { const v = $(id).value; if (v && v !== '请按下组合键…') taken.add(v); });
    return taken;
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    $('setSubtitleEnabled').addEventListener('change', function () {
      if (bridge.setSubtitleEnabled) bridge.setSubtitleEnabled($('setSubtitleEnabled').checked);
    });
