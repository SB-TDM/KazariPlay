/* hook_select.js — Hook 点选择弹窗（V1.1）
 * 触发：启动已启用翻译但无 hook_code 的游戏后（detail.js 收到 need_hook_select）。
...
(function () {
  ...
  function poll() {
    ...
    bridge.getHookCandidates(function (s) {
      let data = { list: [], error: '' };
  ...
  function pick(c) {
    if (busy) return;
    busy = true;
    bridge.selectHook(gameId, c.handle, c.hook_code || '', function () {
      busy = false;
