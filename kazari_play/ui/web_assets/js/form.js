...
function saveForm() {
    ...
    if (App.data.editingId && !data.title.trim()) {
        showFormError('fTitleRow', '标题不能为空');
        return;
    }
    if (!App.data.editingId && !data.exe_path.trim()) {
        showFormError('fExeRow', '请先选择要添加的 exe 文件');
        return;
    }
...
function initMetaSources() {
    ...
    bridge.getMetadataSources(function (s) {
        try {
            metaSources = JSON.parse(String(s || '[]'));
        }
        catch (e) {
            metaSources = [];
        }
...
function renderSrcBar() {
    ...
    const mk = (id, label, icon, on, usable) => {
        ...
        chip.innerHTML = (icon ? `<img src="${esc(icon)}" onerror="this.style.display='none'" alt="">` : '')
            + `<span>${esc(label)}</span>`;
        if (usable)
            chip.onclick = () => { metaSrcMode = id; renderSrcBar(); };
        return chip;
    };
...
function renderCandidates(cands) {
    ...
    box.innerHTML = '';
    if (!cands.length) {
        box.innerHTML = '<div class="cand-empty">未找到结果</div>';
        return;
    }
    cands.forEach(c => {
...
document.getElementById('btnPickExe').onclick = () => {
    if (bridge)
        bridge.selectExe(function (p) { if (p)
            document.getElementById('fExe').value = String(p); });
};
