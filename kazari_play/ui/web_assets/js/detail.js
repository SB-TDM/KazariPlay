...
function openDetail(g) {
    ...
    bridge.getGame(String(g.id), function (s) {
        try {
...
function refreshDetail() {
    ...
    if (rate) {
        rate.dataset.r = String(App.data.currentGame.rating);
        initRateEdit();
    }
    markRunning();
    renderTransRow();
...
function initRateEdit() {
    ...
    for (let n = 1; n <= 5; n++) {
        ...
        s.onclick = () => {
            ...
            App.data.currentGame.rating = n;
            el.dataset.r = String(n);
            initRateEdit();
        };
        el.appendChild(s);
    }
...
document.getElementById('dlgStart').onclick = () => { if (App.data.currentGame)
    launchGame(App.data.currentGame.id); };
document.getElementById('dlgEdit').onclick = () => { if (App.data.currentGame) {
    closeSheet('detailOverlay', true);
    openEdit(App.data.currentGame);
} };
...
document.querySelectorAll('#dlgMoreMenu .item').forEach(it => {
    it.onclick = () => {
        ...
        if (it.dataset.act === 'vndb') {
            bridge.matchVndb(String(g.id));
            toast('开始匹配：' + g.title);
        }
        else if (it.dataset.act === 'open')
            bridge.openFolder(String(g.id));
        else
            showConfirmDialog({
                title: '从库中移除',
