...
function batchPickCollection(mode) {
    ...
    openPicker(mode === 'add' ? '批量添加收藏夹' : (mode === 'move' ? '移动到收藏夹' : '批量移除收藏夹'), items, function (cid) {
        if (mode === 'add') {
            bridge.addGamesToCollection(JSON.stringify(ids), cid);
            toast('已添加到收藏夹');
        }
        else if (mode === 'remove') {
            bridge.removeGamesFromCollection(JSON.stringify(ids), cid);
            toast('已移出收藏夹');
        }
        else {
            bridge.batchMoveToCollection(JSON.stringify(ids), cid);
            toast('已移动');
        }
...
function trackBatchProgress(title) {
    hideBatchProgress();
    ...
    bpTimer = setInterval(function () {
        bridge.getBatchProgress(function (s) {
            let p = {};
            try {
                p = JSON.parse(String(s || '{}'));
            }
            catch (e) { }
            if (!p || !p.running) {
                hideBatchProgress();
            ...
            if (subEl)
                subEl.textContent = `已完成 ${done} / ${total}`;
        });
    }, 600);
}
// ---------- 批量事件绑定 ----------
const batchBtnEl = document.getElementById('batchBtn');
batchBtnEl.onclick = () => {
    ...
    document.querySelectorAll('.card').forEach(c => c.classList.toggle('selected', App.ui.state.selected.has(c.dataset.id)));
};
document.getElementById('btnSelAll').onclick = () => {
    const ids = filterGames(App.data.games).map(g => g.id);
    if (App.ui.state.selected.size === ids.length) {
        App.ui.state.selected.clear();
    }
    else {
        App.ui.state.selected = new Set(ids);
    }
...
document.getElementById('btnBVndb').onclick = () => {
    if (App.ui.state.selected.size === 0)
        return;
