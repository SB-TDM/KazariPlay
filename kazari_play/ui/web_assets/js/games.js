...
function refreshAll(force) {
    ...
    bridge.getCollectionsTree(function (s) {
        App.ui.state.collectionTree = JSON.parse(String(s || '[]'));
...
function filterGames(games) {
    ...
    else if (App.ui.state.nav === '我的收藏')
        list = list.filter(g => g.fav);
    if (App.ui.state.collectionId !== null) {
        ...
        if (App.ui.state.collectionGroupId && App.ui.state.collectionGroupId === App.ui.state.collectionId) {
            ...
            if (grp && grp.children)
                ids = ids.concat(grp.children.map(c => c.id));
        }
    ...
    if (App.ui.state.sort === '名称')
        list.sort((a, b) => a.title.localeCompare(b.title, 'zh'));
    else if (App.ui.state.sort === '评分')
        list.sort((a, b) => b.rating - a.rating);
    else if (App.ui.state.collectionId === null)
    ...
    document.querySelectorAll('.card').forEach(c => {
        ...
        if (c.dataset.id === App.data.runningId) {
            if (!has) {
                ...
                s.className = 'running';
                s.textContent = '运行中';
                const cover = c.querySelector('.cover');
                if (cover)
                    cover.appendChild(s);
            }
        }
...
function toggleSelect(id, card) {
    if (App.ui.state.selected.has(id)) {
        App.ui.state.selected.delete(id);
        card.classList.remove('selected');
    }
    else {
        App.ui.state.selected.add(id);
