...
function bindDrag() {
    ...
    tb.addEventListener('mousedown', e => {
        if (e.target.closest('.winbtn'))
            return;
        dragging = true;
        if (bridge)
            bridge.windowStartDrag(e.screenX, e.screenY);
        e.preventDefault();
    });
    window.addEventListener('mousemove', e => { if (dragging && bridge)
        bridge.windowMoveDrag(e.screenX, e.screenY); });
    window.addEventListener('mouseup', () => { if (dragging) {
        dragging = false;
        if (bridge)
            bridge.windowEndDrag();
    } });
    tb.addEventListener('dblclick', e => { if (!e.target.closest('.winbtn'))
        toggleMax(); });
}
// ---------- 窗口缩放手柄（四边 / 四角自由拉伸） ----------
function bindResize() {
