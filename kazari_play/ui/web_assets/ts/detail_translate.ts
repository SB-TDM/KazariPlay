...
/** 清洗过滤器定义（settings.ts FILTER_DEFS 元素） */
interface CleanFilterDef {
  id: string;
  name: string;
  desc: string;
  agg?: boolean;
...
/** 单游戏清洗配置加载结果（后端 getCleanFilterConfig 返回） */
interface CleanFilterData {
  filters?: { id: string; enabled?: boolean; order?: number }[];
  source?: 'runtime' | 'override' | 'default';
}

function renderTransRow(): void {
  const row = document.getElementById('dlgTransRow');
  if (!row) return;
  const g = App.data.currentGame!;
  row.style.display = 'block';
  const sw = document.getElementById('dlgTrans') as HTMLInputElement;
  ...
  if (g.has_hook_code) {
    ...
    rehook.onclick = () => {
      bridge.getRunning(function (rid: unknown) {
        if (rid && (rid as number | string) === g.id && window.HookSelect) {
...
function loadCleanCfg(gameId: number): void {
  const box = document.getElementById('dlgCleanFilters');
  const defs = window.CLEAN_FILTER_DEFS || [];
  if (!box || !defs.length || !bridge) return;
  bridge.getCleanFilterConfig(String(gameId), function (s: unknown) {
    let data: CleanFilterData = {};
    try { data = JSON.parse(String(s || '{}')) as CleanFilterData; } catch (e) { }
    ...
    const byId: Record<string, { id?: string; enabled?: boolean; order?: number }> = {};
...
function collectCleanCfg(): { id: string; enabled: boolean; order: number }[] {
  const defs = window.CLEAN_FILTER_DEFS || [];
  const checks: Record<string, boolean> = {};
  [...document.querySelectorAll<HTMLInputElement>('#dlgCleanFilters .clean-check')]
    .forEach((c) => { checks[c.dataset.id!] = c.checked; });
  return defs.map((d, i) => ({ id: d.id, enabled: !!checks[d.id], order: i }));
}

function saveCleanCfg(silent?: boolean): void {
