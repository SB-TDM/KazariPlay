...
> **重要提示：实时翻译为实验性功能，可能存在稳定性与翻译质量问题，请谨慎使用。**

**功能**：
- Hook 提取游戏对话文本（基于 Textractor，支持 KRKR / Ren'Py / Unity / RPGMaker 等引擎）
- C++ 内部 AI 翻译（OpenAI 兼容 API，默认 DeepSeek）：先显示原文，AI 翻译完成后替换为译文
- 过滤器链文本清洗（去重 / 注音 / 标签 / 乱码），每游戏独立配置，默认保守策略
- 游戏详情页实时翻译开关（联动字幕窗口显示 / 隐藏）、清洗配置面板（勾选即生效）
- AI 兜底清洗（可选）：确定性清洗不干净时交给 AI 清理
- 字幕窗口跟随游戏窗口底部

**使用隐患（请务必阅读）**：
- **实验性**：Hook 提取依赖游戏引擎与 Hook 点，不同游戏兼容性差异大，可能出现文本抓取不完整、错乱或失败
- **翻译质量不保证**：AI 翻译可能不准确或有误译，人名 / 专有名词可能保留或译错
- **会产生 API 费用**：实时翻译调用第三方 AI API（如 DeepSeek），会产生实际费用，请留意用量
- **文本清洗可能误伤**：去重 / 过滤过滤器可能误伤正常字幕（叠词、短句等）；默认保守策略已降低风险，激进过滤器需在详情页手动开启
- **需要 AI API Key**：需在设置页配置有效的 API Key 才能进行翻译
